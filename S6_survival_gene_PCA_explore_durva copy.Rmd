...
sdata <- sdata[sdata$Durva == 1, ] 
sdata <- sdata[!is.na(sdata$`Sample Name`),]
exprSet <- exprSet[, colnames(exprSet) %in% sdata$`Sample Name`]
timepoint <- "After"
if(timepoint  == "Base"){
  survival_genes <- read.csv("baseline expression with survival_durva.csv")
}
if(timepoint  == "Mid"){
  survival_genes <- read.csv("wk4 expression with survival_durva.csv")
}
if(timepoint  == "After"){
  survival_genes <- read.csv("1mon expression with survival_durva.csv")
}
```
```{r}
sdata <- sdata[,-16:-39]
...
write.csv(coxdata, paste("coxdata_", timepoint, "_durva.csv", sep = ""))
```

# PC1 high vs low KM graph
```{r}
